...
export async function POST(req: Request, ctx: { params: Promise<{ code: string }> }) {
  try {
    // Generous: a whole room joins from one office IP in the same minute.
    const limit = rateLimit(clientKey(req, "join"), 240, 60_000);
    if (!limit.ok) return json({ error: "Too many join attempts. Try again shortly." }, { status: 429 });
