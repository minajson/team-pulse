...
/** Facilitator-only view: adds the moderation queue and per-round progress. */
...
export async function POST(req: Request, ctx: { params: Promise<{ code: string }> }) {
  try {
    ...
    // Wrong-token attempts are throttled hard; correct ones are cheap.
    const limit = rateLimit(`control:${token.slice(0, 8)}`, 300, 60_000);
