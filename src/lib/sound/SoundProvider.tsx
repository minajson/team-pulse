...
/**
 * Wraps a surface in sound. The first pointer/key/touch anywhere in the page
 * unlocks the audio context — the browser requires a gesture, and asking the
 * user to press a "enable sound" button before a session would be friction in
...
export function useSound(): SoundApi {
  ...
  if (ctx) return ctx;
  // Sound is optional decoration; a surface without a provider stays silent
...
/**
 * Fires `cue` whenever `value` changes to something truthy — the common shape
 * for "play a sound when the phase becomes revealed".
 */
export function useCueOnChange<T>(value: T, cue: CueName, shouldPlay: (next: T, prev: T) => boolean) {
  const { play } = useSound();
  ...
  useEffect(() => {
    ...
    // `shouldPlay` is intentionally not a dependency: callers pass an inline
