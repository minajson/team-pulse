...
const STORAGE_KEY = "team-pulse:sound";
 ...
 * is a no-op that costs nothing. Volume and mute live here, so a single
 * facilitator toggle silences the whole app.
 */
export class SoundManager {
  ...
  constructor() {
    if (typeof window !== "undefined") {
      ...
      if (stored === "muted") this.muted = true;
    }
  }
  ...
  /**
   * @param throttleMs Collapses repeats — thirty simultaneous token drops
   *        should sound like a handful of bundles landing, not a hailstorm.
   */
  play(cue: CueName, throttleMs = 0) {
    ...
    trim.gain.value = CUE_GAIN[cue] ?? 0.5;
...
declare global {
  var __teamPulseSound: SoundManager | undefined;
}

export function getSoundManager(): SoundManager {
  globalThis.__teamPulseSound ??= new SoundManager();
  return globalThis.__teamPulseSound;
}
