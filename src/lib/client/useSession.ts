...
const identityKey = (code: string) => `team-pulse:participant:${code}`;
export const facilitatorKey = (code: string) => `team-pulse:facilitator:${code}`;
...
export function writeIdentity(code: string, identity: Identity) {
  writeStored(identityKey(code), JSON.stringify(identity));
}
...
export interface ParticipantApi {
  ...
  submit: (questionId: string, payload: SubmitPayload) => Promise<void>;
  react: (responseId: string) => Promise<void>;
  leave: () => void;
}
...
export function useParticipant(code: string): ParticipantApi {
  ...
  // a join in this tab and a rejoin in another both land without an extra
  // render pass or a stale copy.
  ...
  const submit = useCallback(
    async (questionId: string, payload: SubmitPayload) => {
      setError(null);
      try {
        await post(`/api/sessions/${encodeURIComponent(code)}/respond`, { questionId, ...payload }, authHeaders);
      } catch (e) {
  ...
  const react = useCallback(
    async (responseId: string) => {
      try {
        await post(`/api/sessions/${encodeURIComponent(code)}/react`, { responseId }, authHeaders);
      } catch {
        // A missed heart is not worth an error message on someone's phone.
      }
    },
    [code, authHeaders],
  );
