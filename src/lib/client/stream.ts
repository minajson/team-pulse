 ...
 * facilitator credentials into the query string — into browser history, proxy
 * logs, and any screenshot of the projector's URL bar. Reading the stream by
 * hand costs about forty lines and keeps credentials in headers.
 *
 * Reconnects with capped backoff and reconnects immediately when a phone comes
 * back from sleep, which is the common case: a participant locks their screen
 * during discussion and unlocks it for the next question.
 */
...
function dispatch(frame: string, onEvent: (event: string, data: unknown) => void) {
  ...
  try {
    ...
    // Malformed frame — skip it; the next full state will resync us.
  }
}
