...
/**
 * Zero-config driver: authoritative copy in process memory, snapshotted to a
 * JSON file so a server restart mid-session does not lose the room.
 *
 * Correct for the single-node deployments this product actually runs on (a
 * facilitator running one session in one room). For multi-instance hosting,
 * set TEAM_PULSE_DATABASE_URL and the Postgres driver takes over.
 */
export class MemoryStore implements SessionStore {
  ...
  constructor(dataDir?: string) {
    const dir = dataDir ?? process.env.TEAM_PULSE_DATA_DIR ?? path.join(process.cwd(), ".data");
  ...
  /** Serialise writes per session so concurrent submits cannot interleave. */
  private withLock<T>(code: string, fn: () => Promise<T> | T): Promise<T> {
    ...
    void tail.then(() => {
      // Drop the entry once we are the tail, so the map does not grow.
  ...
  async update<T>(code: string, mutator: Mutator<T>): Promise<UpdateResult<T> | null> {
    ...
    return this.withLock(code, () => {
      ...
      // Work on a clone so an aborted or throwing mutator cannot leave a
